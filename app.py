...
st.markdown('<div class="main-card"><h1 class="title-text">AudioFlow</h1><p class="subtitle-text">Profesionální hudební nástroj</p>', unsafe_allow_html=True)
...
if submit_btn and url_input:
    ...
    if video_id:
        try:
            ...
            # 2. ČÁST: ITUNES DATA (Jen pokud existují)
            if music_meta:
                st.markdown(f'<table class="analysis-table"><tr><td class="label-col">Album</td><td>{music_meta["album"]}</td></tr><tr><td class="label-col">Žánr</td><td>{music_meta["genre"]}</td></tr><tr><td class="label-col">Rok</td><td>{music_meta["year"]}</td></tr></table>', unsafe_allow_html=True)

            # 3. ČÁST: TECHNICKÉ ÚDAJE A SLUŽBY
            search_query = urllib.parse.quote(title)
            st.markdown(f'<table class="analysis-table"><tr><td class="label-col">Kvalita</td><td>320 kbps (HD)</td></tr><tr><td class="label-col">YouTube ID</td><td><code>{video_id}</code></td></tr><tr><td class="label-col">Služby</td><td><a href="https://chordify.net/search/{search_query}" target="_blank" class="service-link chordify">🎸 Akordy</a><a href="https://genius.com/search?q={search_query}" target="_blank" class="service-link genius">📝 Text</a><a href="https://open.spotify.com/search/{search_query}" target="_blank" class="service-link spotify">🎧 Spotify</a></td></tr></table>', unsafe_allow_html=True)
